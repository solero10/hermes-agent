...
class HermesCLI(CLIAgentSetupMixin, CLICommandsMixin):
    ...
    def _read_codex_usage_status(self) -> str:
        """Read the cached OpenAI Codex account-usage line for the status bar.

        This is intentionally file-only.  The hot prompt_toolkit render path
        must never call the OpenAI API or run helper binaries; the background
        ``update-codex-usage-status`` producer refreshes this cache separately.
        """
        now = time.monotonic()
        cached_until = getattr(self, "_codex_usage_status_cached_until", 0.0) or 0.0
        if now < cached_until:
            return getattr(self, "_codex_usage_status_cache", "") or ""

        status = ""
        try:
            raw_path = os.getenv("HERMES_CODEX_USAGE_STATUS_FILE")
            path = Path(raw_path).expanduser() if raw_path else get_hermes_home() / "status" / "codex-usage.txt"
            if path.exists():
                # First line only; producer already writes a single sanitized
                # line, but keep this reader defensive so a bad cache cannot
                # wrap/corrupt the prompt_toolkit footer.
                status = path.read_text(encoding="utf-8", errors="ignore").splitlines()[0].strip()
                status = "".join(ch for ch in status if ch.isprintable())[:160]
        except Exception:
            status = ""

        self._codex_usage_status_cache = status
        self._codex_usage_status_cached_until = now + 30.0
        return status
    ...
    def _get_status_bar_snapshot(self) -> Dict[str, Any]:
        ...
        snapshot = {
            ...
            "session_api_calls": 0,
            "compressions": 0,
            "codex_usage_status": self._read_codex_usage_status(),
