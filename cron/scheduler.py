...
def _is_channel_dm_topic(
    ...
    """
    # Resolve on the CLASS, not the instance (see PITFALLS.md, "Probe classes,
    # not MagicMock instances, for optional adapter capabilities"): a MagicMock
...
def _resolve_job_reasoning_config(job: dict, cfg: dict, model: str = "") -> dict | None:
    ...
    if job_effort is not None:
        parsed = parse_reasoning_effort(str(job_effort).strip().lower())
        if parsed is not None:
            return parsed
    # Empty or malformed stored overrides fail safely back to the configured
    # global/per-model setting instead of silently selecting the provider default.
    return resolve_reasoning_config(cfg if isinstance(cfg, dict) else {}, str(model or ""))
