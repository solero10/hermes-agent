...
def _legacy_removed_credential_markers(auth_store: Dict[str, Any], provider_id: str) -> tuple[set[str], set[str]]:
    """Return legacy removed-credential ids/account ids for one provider.

    v0.19's native write path uses ``removed_ids`` and ``suppressed_sources``.
    Ken's pre-v0.19 auth store can also contain ``credential_pool_removed``
    tombstones. Keep read/write compatible with that legacy marker so an
    upgrade does not resurrect an account Ken manually removed.
    """
    removed_ids: set[str] = set()
    removed_accounts: set[str] = set()
    if not isinstance(auth_store, dict):
        return removed_ids, removed_accounts

    for key in ("credential_pool_removed", "removed_credentials"):
        container = auth_store.get(key)
        if not isinstance(container, dict):
            continue
        items = container.get(provider_id)
        ...
        for item in items:
            ...
            if account_id:
                removed_accounts.add(str(account_id))
    return removed_ids, removed_accounts


def _credential_account_id(entry: Dict[str, Any]) -> Optional[str]:
...
def _credential_matches_removed_marker(
    entry: Any,
    removed_ids: set[str],
    removed_accounts: set[str],
) -> bool:
    if not isinstance(entry, dict):
        return False
    entry_id = entry.get("id")
    if entry_id and str(entry_id) in removed_ids:
        return True
    account_id = _credential_account_id(entry)
    return bool(account_id and account_id in removed_accounts)


def _filter_removed_credential_entries(
    entries: Any,
    provider_id: str,
    *stores: Dict[str, Any],
) -> List[Dict[str, Any]]:
    ...
    removed_accounts: set[str] = set()
    for store in stores:
        ids, accounts = _legacy_removed_credential_markers(store, provider_id)
        removed_ids.update(ids)
        removed_accounts.update(accounts)
    return [
        entry
        for entry in entries
        if isinstance(entry, dict)
        and not _credential_matches_removed_marker(entry, removed_ids, removed_accounts)
    ]


def read_credential_pool(provider_id: Optional[str] = None) -> Dict[str, Any] | List[Dict[str, Any]]:
    ...
    if provider_id is None:
        merged: Dict[str, Any] = {}
        for pool_key in sorted(set(pool) | set(global_pool)):
            local_entries = _filter_removed_credential_entries(
                pool.get(pool_key), pool_key, auth_store, global_store
            )
            if local_entries:
                merged[pool_key] = local_entries
                continue
            global_entries = _filter_removed_credential_entries(
                global_pool.get(pool_key), pool_key, auth_store, global_store
            )
            if global_entries:
                merged[pool_key] = global_entries
            elif pool_key in pool:
                merged[pool_key] = []
        return merged

    provider_entries = _filter_removed_credential_entries(
        pool.get(provider_id), provider_id, auth_store, global_store
    )
    if provider_entries:
        return provider_entries
    # Profile has no entries for this provider — fall back to global.
    return _filter_removed_credential_entries(
        global_pool.get(provider_id), provider_id, auth_store, global_store
    )


def _credential_pool_identity(value: Any) -> str | None:
    text = str(value or "").strip().lower()
    return text or None


def _credential_pool_label(entry: Dict[str, Any]) -> str:
    return str(
        entry.get("label")
        or entry.get("display_label")
        or entry.get("stored_label")
        or ""
    ).strip()
...
def _merge_credential_pool_entries_preserving_disk(
    ...
    Long-running Hermes processes can keep a stale ``CredentialPool`` object
    while another process restores or adds manual accounts.  Disk identity and
    order are the source of truth: incoming rows update matching disk rows,
    missing manual disk rows are preserved, and duplicate stale singleton rows
    from old snapshots are not appended.  Explicit removals are filtered before
    this helper is called via ``removed_ids`` / tombstone checks.
    """
    incoming_dicts = [dict(entry) for entry in incoming_entries if isinstance(entry, dict)]
    current_dicts = [dict(entry) for entry in current_entries if isinstance(entry, dict)]
    ...
    for idx, entry in enumerate(incoming_dicts):
        entry_id = _credential_pool_identity(entry.get("id"))
        if entry_id:
            by_id.setdefault(entry_id, idx)
        source = _credential_pool_identity(entry.get("source"))
    ...
    existing_manual_keys: set[tuple[str | None, str | None]] = set()
    disk_singleton_sources: set[str] = set()
    protect_disk_priority = False
    for disk_entry in current_dicts:
        disk_id = _credential_pool_identity(disk_entry.get("id"))
        if _is_manual_credential_source(disk_entry) and (
            not disk_id or disk_id not in by_id
        ):
            protect_disk_priority = True
            break
    protected_identity_fields = {"id", "label", "source"}
    if protect_disk_priority:
        protected_identity_fields.add("priority")

    for disk_entry in current_dicts:
        disk = dict(disk_entry)
        disk_id = _credential_pool_identity(disk.get("id"))
        disk_source = _credential_pool_identity(disk.get("source"))
        ...
        if incoming_idx is None:
            if _is_manual_credential_source(disk):
                merged.append(disk)
            continue

        incoming = incoming_dicts[incoming_idx]
        used.add(incoming_idx)
        incoming_source = _credential_pool_identity(incoming.get("source"))
    ...
    for entry in merged:
        ...
        if _is_manual_credential_source(entry):
            existing_manual_keys.add(
                (
                    _credential_pool_identity(entry.get("source")),
                    _credential_pool_identity(_credential_pool_label(entry)),
                )
            )
    ...
    for idx, incoming in enumerate(incoming_dicts):
        if idx in used:
            continue
        incoming_id = _credential_pool_identity(incoming.get("id"))
        ...
        if incoming_source and not _is_manual_credential_source(incoming):
            ...
            manual_key = (
                incoming_source,
                _credential_pool_identity(_credential_pool_label(incoming)),
            )
...
def write_credential_pool(
    ...
    *,
    ...
    with _auth_store_lock():
        ...
        legacy_removed_ids, legacy_removed_accounts = _legacy_removed_credential_markers(
            auth_store, provider_id
        )
        removed.update(legacy_removed_ids)
        sanitized_entries = [
            entry
            for entry in sanitized_entries
            if not _credential_matches_removed_marker(entry, removed, legacy_removed_accounts)
        ]
        ...
        existing_list = [
            ...
            if isinstance(entry, dict)
            and not _credential_matches_removed_marker(entry, removed, legacy_removed_accounts)
        ]
        pool[provider_id] = _merge_credential_pool_entries_preserving_disk(
            existing_list,
            sanitized_entries,
        )
