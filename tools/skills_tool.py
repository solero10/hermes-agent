...
#   - signature = each scan dir + category dir mtime plus immediate skill-dir
#     names (bounded to the normal two-level skills/category/skill layout;
#     catches skill add/remove inside categories even when parent mtimes have
#     coarse resolution), plus the disabled-set
...
def _skills_scan_signature(dirs_to_scan, disabled) -> tuple:
    ...
    O(#dirs + #categories + #skill dirs) stat calls, not a full recursive walk.
    Includes the platform the scan's ``skill_matches_platform`` filter will use
    (read from ``agent.skill_utils``'s ``sys`` so test patches of that module
                        ...
                        if entry.is_dir(follow_symlinks=False):
                            ...
                            try:
                                with os.scandir(entry.path) as child_it:
                                    for nested in child_it:
                                        try:
                                            if nested.is_dir(follow_symlinks=False):
                                                grandchildren.append(nested.name)
                                        except OSError:
                                            continue
                            except OSError:
                                pass
                            children.append((
                                entry.name,
                                child_mtime,
                                tuple(sorted(grandchildren)),
                            ))
                    except OSError:
                        continue
        except OSError:
            pass
        sig.append((str(d), root_mtime, tuple(sorted(children))))
