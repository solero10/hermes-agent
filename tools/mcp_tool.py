...
def _mcp_server_needs_connect(server: Any) -> bool:
    """Return True when a cached server record cannot reconnect itself.

    A connected server has ``session`` populated. A server with ``session=None``
    and a still-running task may be mid-reconnect/parked and should not get a
    duplicate connection. A server whose task has exited, however, is a stale
    record and must not make discovery idempotently skip it forever.
    """
    if server is None:
        return True
    ...
    # Avoid MagicMock-style auto-created private attributes: unit-test stubs
    # often omit _task, and getattr(mock, "_task") fabricates a truthy mock
    # whose .done() also fabricates a truthy return value. Only explicit
    # instance attributes should participate in lifecycle decisions. Real
    # MCPServerTask instances are slotted, so vars() is unavailable; for those
    # concrete slotted attributes, direct lookup is still safe because it will
    # raise AttributeError instead of fabricating mocks.
    try:
        explicit_attrs = vars(server)
    except TypeError:
        explicit_attrs = {}

    slots = getattr(type(server), "__slots__", ())
    if isinstance(slots, str):
        slot_names = {slots}
    else:
        slot_names = set(slots or ())

    def _explicit_attr(name: str) -> Any:
        if name in explicit_attrs:
            return explicit_attrs[name]
        if isinstance(server, MCPServerTask) or name in slot_names:
            try:
    ...
    is_recycled_stdio = getattr(server, "_is_recycled_stdio", None)
    if callable(is_recycled_stdio):
        try:
            if is_recycled_stdio():
                return False
        except Exception:
            pass

    # Some parked server tasks have no usable session while their lifecycle
    # coroutine waits on _reconnect_event for the next transport rebuild. Do
    # not replace those records with a duplicate connection attempt: nudge the
    # existing reconnect machinery instead (the handler/register paths call
    # _signal_reconnect). Unit-test stubs may not expose the private _task
    # attribute, so the event itself is the stable capability signal.
    reconnect_event = _explicit_attr("_reconnect_event")
    if reconnect_event is not None and hasattr(reconnect_event, "set"):
        task = _explicit_attr("_task")
...
def _reconnect_mcp_server_if_dead(server_name: str) -> Optional["MCPServerTask"]:
    ...
    logger.info(
        "MCP server '%s' is disconnected with no live reconnect task; attempting lazy reconnect",
        server_name,
    )
    try:
        register_mcp_servers({server_name: config})
    except Exception as exc:
        logger.warning("MCP server '%s' lazy reconnect failed: %s", server_name, exc)
...
def _get_connected_server_for_call(server_name: str) -> Optional[MCPServerTask]:
    """Return a connected server, lazily reconnecting dead/recycled state."""
    with _lock:
        server = _servers.get(server_name)
    if _mcp_server_needs_connect(server):
        return _reconnect_mcp_server_if_dead(server_name)
...
def register_mcp_servers(servers: Dict[str, dict]) -> List[str]:
    ...
    with _lock:
        cached_servers = dict(_servers)
        connecting = set(_server_connecting)
        new_servers = {
            k: v
            for k, v in servers.items()
            if _mcp_server_needs_connect(cached_servers.get(k))
        ...
        stale_cached = [
            cached_servers[k]
            for k in servers
            if k in cached_servers
            and k not in new_servers
            and getattr(cached_servers[k], "session", None) is None
        ]
