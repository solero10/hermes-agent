...
def recover_with_credential_pool(
    ...
    *,
    ...
    if effective_reason == FailoverReason.rate_limit:
        ...
        # OpenAI Codex can report a soft 429 as ``usage_limit_reached`` even
        # when the account still has quota.  If no reset timestamp is supplied,
        # confirm once on the same credential before mutating the pool; the
        # second 429 still rotates through the normal hard-stop path.
