...
# Ken's local policy: never use Gemini 3 Flash Preview as a silent fallback.
# Task-scoped config may still provide better provider/model ladders; these
# constants are only the generic provider fallback safety net.
_OPENROUTER_MODEL = "nvidia/nemotron-3-super-120b-a12b:free"
_NOUS_MODEL = "nvidia/nemotron-3-super-120b-a12b:free"
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    # _NOUS_MODEL (Ken's local non-Gemini default) when the Portal is unreachable
...
def _refresh_nous_recommended_model(
    *, vision: bool, stale_model: Optional[str]
    ...
    model name to retry with:
      ...
      * ``_NOUS_MODEL`` (Ken's local non-Gemini default), if it too
        differs from the failed model.
...
def _get_auxiliary_task_config(task: str) -> Dict[str, Any]:
    ...
    # v15 renamed per-task auxiliary fallback lists from model_fallbacks to
    # fallback_chain. Keep the legacy key readable so sandbox/live config
    # migration can be staged safely instead of breaking side-task fallbacks at
    # first boot. The new key wins when both are present.
    if "fallback_chain" not in task_config and isinstance(task_config.get("model_fallbacks"), list):
        task_config["fallback_chain"] = list(task_config["model_fallbacks"])
